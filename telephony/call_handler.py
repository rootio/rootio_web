...
class CallHandler:
    ...
    def register_for_incoming_calls(self, recipient, is_community_menu=False):
        ...
        gws.sort()
        if is_community_menu:  # register only the first gateway, only 8 digits to match various incoming formats
            if len(gws) > 0:
                self.__incoming_call_recipients[gws[0][-8:]] = recipient
        else:
            for x in range(1, len(gws)):  # skip the first, used by community menu
                self.__incoming_call_recipients[gws[x][-8:]] = recipient
                self.__radio_station.logger.info(
    ...
    def call(self, program_action, to_number, argument, use_sip=False,
        ...
        if to_number in self.__available_calls.keys():
            ...
            if self.__radio_station.station.is_high_bandwidth and use_sip and len(self.__outgoing_sip_gateways) > 0:
                ...
                self.__waiting_call_recipients[str(to_number)[-8:]] = program_action
                ...
                if result is not None and result.split(" ")[0] == "+OK":
                    ...
                    try:
                        del self.__waiting_call_recipients[str(to_number)[-8:]]
                    except:
                        pass
                    return False, None
            else:
                ...
                self.__radio_station.logger.info(
                    "setting up new call for argument '{0}': {1}".format(argument, call_command))
                self.__waiting_call_recipients[str(to_number)[-8:]] = program_action # use -8 here
                result = self.__do_esl_command(call_command)
                self.__radio_station.logger.info("Result of call ESL command is {0}".format(result))
        ...
        while 1:
            ...
            if event_name == "CHANNEL_ANSWER" and 'Caller-Destination-Number' in event_json:
                ...
                try:
                    ...
                    elif str(event_json['Caller-Destination-Number'])[-8:] in self.__waiting_call_recipients:
                        self.__available_calls[str(event_json['Caller-Destination-Number'])[-8:]] = event_json
                        self.__waiting_call_recipients[
                            str(event_json['Caller-Destination-Number'])[-8:]].notify_call_answered(event_json)
                        self.__radio_station.logger.info(
                            "Deleting recipient {0} from waiting call recipients {1}".format(
                                str(event_json['Caller-Destination-Number'])[-8:], self.__waiting_call_recipients))
                        del self.__waiting_call_recipients[str(event_json['Caller-Destination-Number'])[-8:]]
                    ...
                    elif str(event_json['Caller-ANI'])[-8:] in self.__waiting_call_recipients:
                        self.__available_calls[str(event_json['Caller-ANI'])[-8:]] = event_json
                        self.__waiting_call_recipients[
                            str(event_json['Caller-ANI'])[-8:]].notify_call_answered(event_json)
                        self.__radio_station.logger.info(
                            "Deleting recipient {0} from waiting call recipients {1}".format(
                                str(event_json['Caller-ANI'])[-8:], self.__waiting_call_recipients))
                        del self.__waiting_call_recipients[str(event_json['Caller-ANI'])[-8:]]
                except e:
                ...
                try:
                    ...
                    elif str(event_json['Caller-Destination-Number'])[-8:] in self.__incoming_dtmf_recipients:
                        self.__radio_station.logger.info(
                            "Received DTMF [{0}] for recipient {1} in {2}".format(event_json["DTMF-Digit"],
                                                                                  event_json[
                                                                                      'Caller-Destination-Number'],
                                                                                  self.__incoming_dtmf_recipients))
                        threading.Thread(target=self.__incoming_dtmf_recipients[
                            str(event_json['Caller-Destination-Number'])[-8:]].notify_incoming_dtmf,
                                         args=(event_json,)).start()
                    ...
                    # And now those registered using the call from number
                        ...
                        threading.Thread(target=self.__incoming_dtmf_recipients[
                            str(event_json['Caller-ANI'])[-9:]].notify_incoming_dtmf,
                                         args=(event_json,)).start()
                    elif str(event_json['Caller-ANI'])[-8:] in self.__incoming_dtmf_recipients:
                        self.__radio_station.logger.info(
                            "Received DTMF [{0}] for recipient {1} in {2}".format(event_json["DTMF-Digit"],
                                                                                  event_json[
                                                                                      'Caller-ANI'],
                                                                                  self.__incoming_dtmf_recipients))
                        threading.Thread(target=self.__incoming_dtmf_recipients[
                            str(event_json['Caller-ANI'])[-8:]].notify_incoming_dtmf,
                                         args=(event_json,)).start()
                ...
                try:
                    ...
                    if str(event_json['Caller-Destination-Number'])[-9:] in self.__available_calls:
                        ...
                        loggable = True
                    elif str(event_json['Caller-Destination-Number'])[-8:] in self.__available_calls:
                        self.__radio_station.logger.info("Removing call to {0} from available calls {1}".format(
                            str(event_json['Caller-Destination-Number'])[-8:], self.__available_calls.keys()))
                        del self.__available_calls[str(event_json['Caller-Destination-Number'])[-8:]]
                        self.__release_gateway(event_json)
                        loggable = True
                    ...
                    if str(event_json['Caller-Destination-Number'])[-9:] in self.__call_hangup_recipients:
                        ...
                        loggable = True
                    elif str(event_json['Caller-Destination-Number'])[-8:] in self.__call_hangup_recipients:
                        self.__call_hangup_recipients[
                            str(event_json['Caller-Destination-Number'])[-8:]].notify_call_hangup(event_json)
                        loggable = True

                    # remove the call from the list of available calls
                    if str(event_json['Caller-ANI'])[-9:] in self.__available_calls:
                        self.__radio_station.logger.info("Removing call to {0} from available calls {1}".format(
                            str(event_json['Caller-ANI'])[-9:], self.__available_calls.keys()))
                        del self.__available_calls[str(event_json['Caller-ANI'])[-9:]]
                        self.__release_gateway(event_json)
                        loggable = True
                    elif str(event_json['Caller-ANI'])[-8:] in self.__available_calls:
                        self.__radio_station.logger.info("Removing call to {0} from available calls {1}".format(
                           str(event_json['Caller-ANI'])[-8:], self.__available_calls.keys()))
                        del self.__available_calls[str(event_json['Caller-ANI'])[-8:]]
                        self.__release_gateway(event_json)
                        loggable = True
                    ...
                    #  And now those registered using the from number of the caller
                        ...
                        loggable = True
                    elif str(event_json['Caller-ANI'])[-8:] in self.__call_hangup_recipients:
                        self.__call_hangup_recipients[
                            str(event_json['Caller-ANI'])[-8:]].notify_call_hangup(event_json)
                        loggable = True
            ...
            # This should use 8 digits not 9!
                ...
                try:
                    self.__radio_station.logger.info("Notifying park recipient for {0} in {1} and {2}, {3}".format(
                        event_json['Caller-Destination-Number'][-8:], self.__incoming_call_recipients,
                        self.__host_call_recipients, self.__community_ivr_number[-8:]))
                    if self.__community_ivr_number is not None and event_json['Caller-Destination-Number'][-8:] == self.__community_ivr_number[-8:]: # Someone calling into the community
                        ...
                        self.__community_menu_sessions[event_json['Caller-ANI'][-8:]] = community_menu
                        threading.Thread(target=self.__community_menu_sessions[event_json['Caller-ANI'][-8:]].notify_incoming_call,
                                         args=(event_json,)).start()
                    elif event_json['Caller-ANI'][-8:] in self.__host_call_recipients:
                        self.__host_call_recipients[event_json['Caller-ANI'][-8:]].notify_host_call(event_json)
                       ...
                       -8:] in self.__incoming_call_recipients:  # Someone calling into a talk show
                        threading.Thread(target=self.__incoming_call_recipients[
                            str(event_json['Caller-Destination-Number'])[-8:]].notify_incoming_call,
                                         args=(event_json,)).start()
                    elif event_json['Caller-Destination-Number'][
                    ...
                    if event_json['Caller-Destination-Number'][-9:] in self.__media_playback_start_recipients:
                        ...
                        threading.Thread(target=self.__media_playback_start_recipients[
                            str(event_json['Caller-Destination-Number'])[-9:]].notify_media_play_start,
                                         args=(event_json,)).start()
                        # del self.__media_playback_stop_recipients[event_json['Caller-Destination-Number']]
                    elif event_json['Caller-Destination-Number'][-8:] in self.__media_playback_start_recipients:
                        self.__radio_station.logger.info(
                            "Notifying media playback start recipient for {0} in {1}".format(
                                event_json['Caller-Destination-Number'], self.__media_playback_start_recipients))
                        threading.Thread(target=self.__media_playback_start_recipients[
                            str(event_json['Caller-Destination-Number'])[-8:]].notify_media_play_start,
                                         args=(event_json,)).start()
                        # del self.__media_playback_stop_recipients[event_json['Caller-Destination-Number']]
                    ...
                    #  And now those registered using the from number of the call
                        ...
                        threading.Thread(target=self.__media_playback_start_recipients[
                            str(event_json['Caller-ANI'])[-9:]].notify_media_play_start,
                                         args=(event_json,)).start()
                        # del self.__media_playback_stop_recipients[event_json['Caller-Destination-Number']]
                    elif event_json['Caller-ANI'][-8:] in self.__media_playback_start_recipients:
                        self.__radio_station.logger.info(
                            "Notifying media playback start recipient for {0} in {1}".format(
                                event_json['Caller-ANI'], self.__media_playback_start_recipients))
                        threading.Thread(target=self.__media_playback_start_recipients[
                            str(event_json['Caller-ANI'])[-8:]].notify_media_play_start,
                                         args=(event_json,)).start()
                        # del self.__media_playback_stop_recipients[event_json['Caller-Destination-Number']]
                    ...
                    if event_json['Caller-Destination-Number'][-9:] in self.__media_playback_stop_recipients:
                        ...
                        threading.Thread(target=self.__media_playback_stop_recipients[
                            str(event_json['Caller-Destination-Number'])[-9:]].notify_media_play_stop,
                                         args=(event_json,)).start()
                        # del self.__media_playback_stop_recipients[event_json['Caller-Destination-Number']]
                    elif event_json['Caller-Destination-Number'][-8:] in self.__media_playback_stop_recipients:
                        # self.__radio_station.logger.info("got media stop bug as {0}".format(event_json_string))
                        self.__radio_station.logger.info(
                            "Notifying media playback stop recipient for {0} in {1}".format(
                                event_json['Caller-Destination-Number'], self.__media_playback_stop_recipients))
                        threading.Thread(target=self.__media_playback_stop_recipients[
                            str(event_json['Caller-Destination-Number'])[-8:]].notify_media_play_stop,
                                         args=(event_json,)).start()
                        # del self.__media_playback_stop_recipients[event_json['Caller-Destination-Number']]
                    ...
                    # And now those registered using the from number of the caller
                        ...
                        threading.Thread(target=self.__media_playback_stop_recipients[
                            str(event_json['Caller-ANI'])[-9:]].notify_media_play_stop,
                                         args=(event_json,)).start()
                        # del self.__media_playback_stop_recipients[event_json['Caller-Destination-Number']]
                    elif event_json['Caller-ANI'][-8:] in self.__media_playback_stop_recipients:
                        # self.__radio_station.logger.info("got media stop bug as {0}".format(event_json_string))
                        self.__radio_station.logger.info(
                            "Notifying media playback stop recipient for {0} in {1}".format(
                                event_json['Caller-ANI'], self.__media_playback_stop_recipients))
                        threading.Thread(target=self.__media_playback_stop_recipients[
                            str(event_json['Caller-ANI'])[-8:]].notify_media_play_stop,
                                         args=(event_json,)).start()
                        # del self.__media_playback_stop_recipients[event_json['Caller-Destination-Number']]
                except e:
                    self.__radio_station.logger.error('error in media bug stop: {0}'.format(e.message))
    ...
    def __release_gateway(self, event_json):
        ...
        for i in range(7, 13, 1):  # Phone numbers can be from 7 - 9 digits
            try:

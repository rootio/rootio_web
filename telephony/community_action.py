...
class CommunityAction:
    ...
    def __stop_media(self, event_json):  # stop the media being played by the player
        try:
            ...
            self.__call_handler.deregister_for_call_hangup(event_json['Caller-Destination-Number'][-9:])
            self.__call_handler.deregister_for_media_playback_stop(event_json['Caller-Destination-Number'][-9:])

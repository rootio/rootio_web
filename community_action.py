...
class CommunityAction:
    ...
    def __load_track(self):  # load the media to be played
        self.__content = self.program.db.query(CommunityContent).filter(
            CommunityContent.type_code == self.__type_code).filter(
            CommunityContent.station_id == self.program.radio_station.station.id).all()
    ...
    def __request_call(self):
        return self.__call_handler.call(self, self.program.radio_station.station.transmitter_phone.number, 'play',
                                        self.__type_code, self.duration)

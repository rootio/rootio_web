...
from wtforms import StringField, SelectField, SubmitField, FormField, TextField, TextAreaField, HiddenField, RadioField, \
    IntegerField
...
from .models import Station, StationAnalytic, Program, ProgramType, ScheduledBlock, Person, Language, Location, Network
...
NetworkFormBase = model_form(Network, db_session=db.session, base_class=Form,
                             exclude=['created_at', 'updated_at', 'paddingcontents'])


class NetworkForm(NetworkFormBase):
    submit = SubmitField(_('Save'))
    field_order = ('name', 'about', '*')
...
StationFormBase = model_form(Station, db_session=db.session, base_class=OrderedForm,
                             field_args={
                                 'network': {'validators': [HasInlineForm, ]},
                                 ...
                                 'cloud_phone': {'description': _('Station contact telephone number'),
                                                 'validators': [HasInlineForm, ]},
                                 'transmitter_phone': {'description': _('Station contact telephone number'),
                                                       'validators': [HasInlineForm, ]},
...
class StationForm(StationFormBase):
    cloud_phone_inline = InlineFormField(PhoneNumberForm, description='/telephony/phonenumber/add/ajax/')
    transmitter_phone_inline = InlineFormField(PhoneNumberForm, description='/telephony/phonenumber/add/ajax/')
    location_inline = InlineFormField(LocationForm, description='/radio/location/add/ajax/')
    network_inline = InlineFormField(NetworkForm, description='/radio/network/add/ajax')
...
LanguageFormBase = model_form(Language, db_session=db.session, base_class=Form,
                              only=['name', 'iso639_1', 'iso639_2',
                                    'locale_code'], )  # explicitly include only these fields, so we don't have to exclude m2m fks

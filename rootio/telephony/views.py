...
def index():
    ...
    summary_query = 'select radio_station.name "station", (select count(*) from telephony_message where ' \
                    'telephony_message.station_id = radio_station.id) "messages", (select count(*) from ' \
                    'telephony_call where telephony_call.station_id = radio_station.id) "calls", (select count(*) ' \
                    'from radio_incominggateway where radio_incominggateway.station_id = radio_station.id) ' \
                    '"incoming_gateways", (select count(*) from radio_outgoinggateway where ' \
                    'radio_outgoinggateway.station_id = radio_station.id)  "outgoing_gateways" from radio_station  ' \
                    'join radio_network on radio_station.network_id = radio_network.id join radio_networkusers on ' \
                    'radio_network.id = radio_networkusers.network_id join user_user on radio_networkusers.user_id = ' \
...
def phonenumber_definition(phonenumber_id):
...
def phonenumber_add_inline():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(phonenumber)
        db.session.commit()
        response = {'status': 'success', 'result': {'id': phonenumber.id, 'string': unicode(phonenumber)},
                    'status_code': 200}
    elif request.method == "POST":
...
def message_records():
    ...
    records = jquery_dt_paginator.get_records(recent_messages,
                                              [Message.text, Message.from_phonenumber, Message.to_phonenumber,
                                               Station.name], request)
    return jsonify(records)

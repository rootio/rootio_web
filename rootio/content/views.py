...
def list_tracks():
    ...
    if show_only:
        tracks = db.session.query(ContentTrack).join(ContentTrack.networks).filter(ContentTrack.deleted != True).filter_by(id = show_only)
    else:
...
def content_upload_add():
    ...
    if request.method == 'POST':
        ...
        if form.validate_on_submit():
            ...
            uri = save_uploaded_file(uploaded_file, upload_directory, file_name=uploaded_file_name, process_audio=False)

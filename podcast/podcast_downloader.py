        ...
        for podcast in podcasts:
            ...
            for link in podcast.links:
                if link.type == u'audio/mpeg':
                    urllib.urlretrieve(link.href, os.path.join(DefaultConfig.CONTENT_DIR, 'podcast', str(self.__podcast.id), podcast.title))
                    self.__log_podcast_download(podcast.title, podcast.itunes_duration, podcast.title, podcast.summary, datetime.fromtimestamp(mktime(podcast.published_parsed)))

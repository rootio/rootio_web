...
class CereprocRestAgent:
    ...
    def __init__(self, cereproc_url, cereproc_username, cereproc_password, gender = 'male', language = 'english', sample_rate = '16000', audio_format = 'wav'):
        ...
        #To save time in TTS operations, it makes sense to load the voices when this class is instantiated rather than for individual TTS requests
        self._voices = self.cprc_list_voices()
 
    
    def _do_cprc_request(self, data): #raise error if http error, if network failure
        headers = {'Content-type': 'text/xml'}
        response = requests.post(self._cereproc_url, headers = headers, data = data)
        return etree.fromstring(response.text)


    def _generate_request_xml(self, root, data):
    ...
    def cprc_list_voices(self):
        requestxml = self._generate_request_xml("listVoices", {'accountID':self._cereproc_username, 'password':self._cereproc_password})
    ...
    def get_cprc_tts(self, text, voice = None, sample_rate = None, audio_format = None, metadata = True):
        ...
        if sample_rate == None: #use default
        ...
        if audio_format == None: #use the default 
            audio_format = self._audio_format
        elif audio_format not in ['wav', 'mp3', 'ogg']: #a format was specified but is not among those that are supported
            raise ValueError("Audio format not supported")
        metadata = bool(metadata)

        requestxml = self._generate_request_xml("speakExtended", {'accountID':self._cereproc_username, 'password':self._cereproc_password, 'voice':voice, 'audioFormat':audio_format, 'sampleRate': sample_rate, 'audio3D':'', 'metadata':metadata, 'text': text})
